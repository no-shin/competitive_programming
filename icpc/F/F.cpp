...
int main() {
  ...
  while (1) {
    ...
    const int xn = ssize(x);
    const int yn = ssize(y);
    map<pair<int, int>, int> mp;
    int idx{};
    rep(i, 0, xn) rep(j, 0, yn) { mp[{x[i], y[j]}] = idx++; }
    // output(mp);
    const int N = ssize(mp);
    vector<pii> invmp;
    for (auto [p, _] : mp) {
    ...
    rep(i, 0, N) {
      ...
      if (check(i, i + 1)) {
        ...
        G[i].emplace_back(i + 1, abs(yi - yi1));
      }
      if (check(i, i - 1)) {
        auto [xi1, yi1] = invmp[i - 1];
        G[i].emplace_back(i - 1, abs(yi - yi1));
      ...
      if (check(i, i + yn)) {
        ...
        G[i].emplace_back(i + yn, abs(xi - xi1));
      }
      if (check(i, i - yn)) {
        auto [xi1, yi1] = invmp[i - yn];
        G[i].emplace_back(i - yn, abs(xi1 - xi));
      }
    }
    ...
    auto kinbou = [&](pii p) {
      ...
      vector<pii> res;
      vector<int> X, Y;
      if (*ranges::lower_bound(x, px) == px) {
        X.emplace_back(px);
      } else {
        auto ue = ranges::lower_bound(x, px);
        if (ue != x.end())
          X.emplace_back(*ue);
        if (ue != x.begin()) {
          ue--;
      ...
      if (*ranges::lower_bound(y, py) == py) {
        Y.emplace_back(py);
      } else {
        auto migi = ranges::lower_bound(y, py);
        if (migi != y.end())
          Y.emplace_back(*migi);
        if (migi != y.begin()) {
          migi--;
    ...
    vector kinbousitenn = kinbou({u, v});
    ...
    while (que.size()) {
      ...
      if (dist[p] < d)
        continue;
      for (auto q : G[p]) {
        if (chmin(dist[q.first], d + q.second))
          que.push({dist[q.first], invmp[q.first]});
      }
      cnt++;
      // if(cnt>=1e5){
      //     output(que);
    ...
    while (Q--) {
      ...
      auto v_ = kinbou({a, b});
      ll ans = infl;
      for (auto aaa : kinbousitenn)
        for (auto bbb : v_) {
          if (aaa == bbb)
            chmin(ans, abs(u - a) + abs(v - b));
        }
      for (auto [ex, ey] : v_) {
